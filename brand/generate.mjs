...
//   3. Icon     512 x 512     metallic "A" whose crossbar IS the red line,
//                             bisecting a faint dark-bronze ring.
//
...
function buildIcon() {
  ...
  const SW = capH * 0.132;
  const aw = capH * 0.82;
  const X = cx - aw / 2; // left of A
  const Yt = cy - capH / 2; // top of A
  const baseY = Yt + capH;
  const apexX = cx;
  const vNotch = SW * 1.5;

  // A legs only (Λ polygon) — crossbar is the red line
  const lambda = poly([
    [X, baseY],
    [apexX, Yt],
    [X + aw, baseY],
  ...
  const subs = [{ d: lambda, eo: false }];
  const off = Math.max(1.5, capH * 0.018);
  const aHi = `<g transform="translate(0,${n(-off)})">${fillPaths(
    subs,
    C.metalTop
  )}</g>`;
  const aSh = `<g transform="translate(0,${n(off)})">${fillPaths(
    subs,
    "#000000"
  )}</g>`;
  const aFace = fillPaths(subs, "url(#i-metal)");

  // crossbar = red line, bridging the inner faces of the legs at yb
  const yb = Yt + capH * 0.6;
  const t = (baseY - yb) / (baseY - (Yt + vNotch));
  const ilx = X + SW + (apexX - (X + SW)) * t;
  const irx = X + aw - SW + (apexX - (X + aw - SW)) * t;
  const line = redLine(ilx - SW * 0.4, irx + SW * 0.4, yb + SW / 2, 13, "i-line");
  ...
  const defs =
    vignette(0.5, 0.5, 0.7) +
    linGradV("i-metal", Yt, baseY, metalStops) +
    ringGrad +
    line.defs;

  const body =
    background(W, H) + rings + aSh + aHi + aFace + line.svg;

  const probes = [
    { x: 256, y: 60, kind: "bg", label: "background" },
    { x: cx, y: Math.round(yb + SW / 2 + 3), kind: "red", label: "red crossbar" },
    { x: 160, y: 376, kind: "metal", label: "A monogram metal" },
  ];
  const scans = [
